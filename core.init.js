...
jQuery(document).ready(function() {
    ...
    //map popup info
    jQuery("#oregon, #ca, #idaho, #mn, #wv, #md, #dc, #nj, #chicago").hover(function(e) {
	  jQuery('#info-box').css('display','block');
	  jQuery('#info-box').html(jQuery(this).data('info'));
	});

	jQuery("#oregon, #ca, #idaho, #mn, #wv, #md, #dc, #nj, #chicago").mouseleave(function(e) {
	...
	jQuery(document).mousemove(function(e) {
      ...
      //replace All posts with Blogs
      jQuery('.page_title').each(function() {
           var text = jQuery(this).text();
          jQuery(this).text(text.replace('All Posts', 'Blogs'));
      });
	
      //Replace Bridging the Gap text with Partners
	jQuery('.page-id-256 .page_title').each(function() {
           var text = jQuery(this).text();
          jQuery(this).text(text.replace('Bridging the Gap', 'Partners'));
      });
